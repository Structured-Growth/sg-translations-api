...
import { ClientUpdateTranslationBodyInterface } from "../../interfaces/client-update-translation-body.interface";
...
import { ClientUpdateTranslationParamsValidator } from "../../validators/client-update-translation-params.validator";
...
export class TranslationSetController extends BaseController {
	...
	async getTranslationSet(
		...
		@Path() locale: string,
		@Queries() query: ClientGetLocalizedTranslationParamsInterface
	): Promise<{ [key: string]: any }> {
		return await this.clientService.getLocalizedTranslation(clientId, locale, query);
	}
	...
	/**
	 * Update a translation set for the specified client
	 */
	@OperationId("Update a set")
	...
	async updateTranslation(
		@Path() clientId: number,
		@Queries() query: {},
		@Body() body: ClientUpdateTranslationBodyInterface
	): Promise<void> {
		await this.clientService.updateFromJSON(clientId, body);
		this.response.status(204);
	}
}
